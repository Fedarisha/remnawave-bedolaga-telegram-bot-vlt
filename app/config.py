...
class Settings(BaseSettings):
    ...
    # SUPPORT_USERNAME is the upstream name and accepts either a Telegram
    # username or an external helpdesk URL. Keep SUPPORT_TG_USERNAME as a
    # fallback for existing VLT deployments that still use the old variable.
    ...
    NALOGO_QUEUE_MAX_ATTEMPTS: int = 72  # Максимум попыток отправки чека (72 × 10мин = 12 часов); <= 0 отключает лимит
    ...
    def _clean_support_contact(self) -> str:
        contact = self.SUPPORT_USERNAME if self.SUPPORT_USERNAME is not None else self.SUPPORT_TG_USERNAME
