...
async def confirm_tariff_purchase(
    ...
    # In multi-tariff mode, buying a tariff creates a NEW subscription — the same
    # tariff twice is a supported case here, so no tariff-level lookup. Renewal is
    # explicit: preview pins the exact row in FSM, which is also race-resistant
    # against a concurrent panel webhook flipping that row's status mid-flow.
    if settings.is_multi_tariff_enabled():
        ...
        if _pinned_sub_id:
            ...
            # Defence: if admin/user switched tariff between preview and confirm,
            # the pinned sub may no longer match — ignore it rather than extend a
            # subscription of the wrong tariff.
