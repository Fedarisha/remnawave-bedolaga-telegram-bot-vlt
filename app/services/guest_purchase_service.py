...
async def _create_nalogo_receipt_for_purchase(
    ...
    try:
        ...
        receipt_uuid = await nalogo_service.create_receipt(
            ...
            receipt_delivery_email=receipt_email,
        ...
        if receipt_uuid:
            ...
            if receipt_url:
                ...
                # Адресат — покупатель, а не одаряемый: для подарка user это
                # получатель подписки, а чек по 422-ФЗ обязан дойти до плательщика
                # (и не должен раскрывать ему сумму покупки).
                await deliver_nalogo_receipt(
                    receipt_url=receipt_url,
                    receipt_uuid=receipt_uuid,
                    telegram_id=receipt_telegram_id,
                    email=receipt_email,
