...
class NaloGoService:
    ...
    async def _save_pending_verification(
        ...
        Адрес и язык доставки сохраняем вместе с остальным: после ручной
        пересылки из админки чек надо доставить покупателю, а к тому моменту
    ...
    async def retry_pending_receipt(self, payment_id: str, bot: Any = None) -> str | None:
        ...
        if receipt_uuid:
            ...
            if receipt_url:
                ...
                # Чек в ФНС уже создан и убран из очереди проверки. Ошибку доставки
                # только логируем: если пробросить её, админ увидит сбой и нажмёт
                # «повторить» — а повтор выпишет ВТОРОЙ чек на тот же платёж.
                try:
                    ...
                    logger.error(
                        'Чек создан, но доставить его покупателю не удалось',
                        payment_id=payment_id,
                        receipt_uuid=receipt_uuid,
