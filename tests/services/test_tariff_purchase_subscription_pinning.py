...
def test_confirm_tariff_purchase_reads_target_subscription_id_from_fsm() -> None:
    ...
    # Форк: тариф-левелового фоллбека здесь НЕТ. Покупка тарифа в мульти-режиме
    # всегда создаёт НОВУЮ подписку (тот же тариф дважды — поддержанный кейс),
    # а продление всегда приходит с пином из FSM. Возврат фоллбека молча
    # превратил бы повторную покупку в продление существующей подписки.
    assert 'get_subscription_by_user_and_tariff(' not in body, (
        'confirm_tariff_purchase must NOT fall back to a (user, tariff) lookup: in this '
        'fork a tariff purchase creates a new subscription unless FSM pins an explicit '
