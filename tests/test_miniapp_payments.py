...
async def test_purchase_tariff_creates_new_subscription_for_same_tariff_in_multi_mode(monkeypatch):
    ...
    # pricing_engine больше не атрибут модуля miniapp — эндпоинт импортирует его
    # локально, поэтому патчим сам синглтон.
    from app.services import pricing_engine as pricing_engine_module

    monkeypatch.setattr(pricing_engine_module.pricing_engine, 'calculate_tariff_purchase_price', fake_calculate)
