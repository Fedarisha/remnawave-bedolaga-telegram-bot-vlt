...
def test_cabinet_button_group_is_startapp_deeplink(cabinet_settings):
    ...
    assert btn is not None
    assert btn.url == 'https://telegram.me/mybot/cabinet?startapp=admin_ticket_42'
...
def test_keyboard_places_cabinet_button_on_top():
    cab = InlineKeyboardButton(text='🗂 Кабинет', url='https://telegram.me/mybot/cabinet?startapp=admin_ticket_7')
    kb = get_ticket_notification_keyboard(7, user_id=42, is_admin=True, cabinet_button=cab)
    # First row is the cabinet button.
    assert kb.inline_keyboard[0][0].url == 'https://telegram.me/mybot/cabinet?startapp=admin_ticket_7'
